 ...
 */
@WebServlet("/HResetPassServ")
public class HResetPassServ extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HResetPassServ() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(oldpass.equals(old))
			{
				...
				{
					response.sendRedirect("Health/ResetPass.jsp?err=1");
				}
				
			}
			else
			{
				response.sendRedirect("Health/ResetPass.jsp?msg=1");
			}
